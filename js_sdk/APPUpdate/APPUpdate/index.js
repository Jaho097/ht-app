...
// 发起ajax请求获取服务端版本号
const getServerNo = function(version,upgrade,isPrompt = false, callback) {
	let httpData = {
		version: version.versionCode,
        versionName: version.versionName
	};
	//let res = this.upgrade;
	//console.log('版本输出：',upgrade);
	if (upgrade) {
		callback && callback({
			versionCode : upgrade[platform+'_version'],//版本号
			versionName : upgrade[platform+'_version_name'],//版本名称
			versionInfo : upgrade[platform+'_description'],//版本信息
			forceUpdate : upgrade[platform+'_power'],//是否强制更新
			downloadUrl : upgrade[platform+'_down']//版本下载链接
		});
	}else{
		uni.showToast({
			title: "暂无新版本",
...
export default function(isPrompt = false,upgrade) {
	getCurrentNo(versionInfo => {
		getServerNo(versionInfo,upgrade,isPrompt, res => {
			console.log(res)
