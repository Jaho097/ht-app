<template>
	<view class="page">
		...
		<view class="goods-discounts">
			<!-- <view class="list">
				<view class="title br-red" :class="{'action':Btype==1}" @click="Btype=1;onPriceList(0)"><text>买入</text>
				</view>
				<view class="title br-green" :class="{'action':Btype==2}" @click="Btype=2;onPriceList(0)">
					<text>卖出</text></view>
			</view> -->
			<div class="sc-daURTG ScOSY">
				<div class="sc-bXGyLb icUOfo">
					<div class="stock">
						<div class="name">{{marketData.name||'-'}} </div>
						<div class="code">{{marketData.code||'0.00'}}</div>
					</div>
					<div class="price"><span class="sc-fjdhpX cEvEJT">{{marketData.current_price||'0.00'}}</span></div>
					<div class="range"><span class="sc-fjdhpX cEvEJT"> 0.01</span></div>
					<div class="rate"><span class="sc-fjdhpX cEvEJT"> 0.31%</span></div>
				</div><a style="text-decoration: none;" @click="toSearch()" class="sc-lkqHmb geobGM" href="javascript:void(0);">
			...
			<view class="price-info flex grid col-4">
				<view class="flex-twice text-left">
					<view class="flex-sub text-sm">
						...
						<text class="right red"
							v-if="marketData.price_range>0">{{marketData.current_price||'0.00'}}</text>
			...
			<view class="attr-size flex">
				<view class="attr-list flex-twice">
					<view class="cu-btn flex-sub" v-for="(item,index) in PriceList"
						:class="{'action':PriceIndex === index}" @click.stop="onPriceList(index)" :key="index">
			...
			<view class="attr-maine">
				<view class="attr-number">
					...
					<view class="number text-center">
						<uni-icons custom-prefix="iconfont" type="icon-jianshao" size="32" color="#03AA3A"
							@click="onReduction('price')"></uni-icons>
						...
						<uni-icons custom-prefix="iconfont" type="icon-zengjia" size="24" color="#FF4500"
							@click="onIncrease('price')"></uni-icons>
					</view>
				</view>
				<view class="attr-number text-center" v-if="Btype === 1">
					...
					<view class="number">
						...
						<uni-icons custom-prefix="iconfont" type="icon-jianshao" size="32" color="#03AA3A"
							@click="onReduction('count')"></uni-icons>
						...
						<uni-icons custom-prefix="iconfont" type="icon-zengjia" size="24" color="#FF4500"
							@click="onIncrease('count')"></uni-icons>
					</view>
				</view>
			...
			<view class="attr-size">
				<view class="attr-list">
					<view class="size-list">
						<uni-tag v-for="(item,index) in AttrList" :key="index" :class="{'on':AttrIndex === index}"
							@click="onAttrList(index)" class="date-tags" :text="item.size" :circle="true" size="small">
						</uni-tag>
					</view>
				</view>
			</view>
			<view class="uni-py-10 uni-px-8" style="width: 100%;">
				<button type="warn" style="width:100%;" class="uni-radius-5" v-if="Btype === 1"
					@click="onConfirm(Btype)">确认买入</button>
				<button type="warn" style="width:100%;background-color: #03AA3A;" class="uni-radius-5"
					v-if="Btype === 2" @click="onConfirm(Btype)">确认卖出</button>
			</view>
		</view>
		<!-- 持仓 -->
	...
	<!-- 	<uni-countdown :show-day="false" :hour="0" :minute="0" :second="30" :reset="true" @timeup="getMarket(code);">
		</uni-countdown> -->
...
<script>
	...
	export default {
		...
		onShow() {
			...
			console.log(this.accounId,'accounIdaccounIdaccounId');
		...
		methods: {
			...
			/*获取交易账户信息*/
			async getSubAccount() {
				http.post(this.apiServer + '/market/index/getSubAccount', {
					token: this.token
				}, {
					header: {
						'content-type': "application/x-www-form-urlencoded"
			...
			async getacInfo(id) {
				console.log('getacInfo')
				uni.showLoading({
					'title': "加载中"
				});
				http.post(this.apiServer + '/market/trade/account_info', {
					token: this.token,
					id: id
				}, {
					header: {
			...
			/*获取可卖数量*/
			async getcanSell(id) {
				http.post(this.apiServer + '/market/trade/canSellcount', {
					id: id,
					token: this.token,
					code: this.code
				}, {
			...
			async getPosition(id) {
				http.post(this.apiServer + '/market/trade/position', {
					id: id,
					token: this.token
				}, {
					header: {
