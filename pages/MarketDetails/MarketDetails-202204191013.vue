<template>
	<view class="pageWrap" id="pageWrap">
		...
		<!-- 价格相关 -->
		<view class="banner-title" id="changeType">
			<!-- 价格 -->
		...
		<view class="klineWrap">
			...
			<view class="btnGroup" id="dayCountBtnGroup" v-show="isShowMinuteChart">
				<view class="btn" 
				:class="{active:indexMinute === index}" 
				v-for="(item,index) in minuteDayCountAry" 
				:key='item.ID' @click="ChangeMinuteDayCount(item.ID,index)">
				{{item.Name}}
				</view>
			</view>
			...
			<!-- 走势图：MACD、KDJ、DMI、ROC -->
			<view class="btnGroup" id="minuteIndexBtnGroup" v-show="isShowMinuteChart">
				<view class="btn" 
					:class="{active:minuteIndexNumber === index}" 
					v-for="(item,index) in minuteIndexAry" 
					:key='index' 
					@click="ChangeMinuteIndex(index, item)"
				>
						{{item.Name}}
				</view>
			</view>
